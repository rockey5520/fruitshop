...
# -mod vendor 
RUN CGO_ENABLED=0 GOOS=linux go build -ldflags="-w -s" -o /go/bin/fruitshop -mod vendor main.go 
...
COPY --from=builder /go/bin/fruitshop /go/bin/fruitshop

# Use an unprivileged user.
USER appuser:appuser

# Run the hello binary.
ENTRYPOINT ["/go/bin/fruitshop"]
