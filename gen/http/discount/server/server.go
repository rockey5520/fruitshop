...
// New instantiates HTTP handlers for all the discount service endpoints using
...
func New(
	...
	return &Server{
		Mounts: []*MountPoint{
			{"Get", "GET", "/server/api/v1/discount/{userId}"},
		},
...
func (s *Server) Service() string { return "discount" }
...
func MountGetHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("GET", "/server/api/v1/discount/{userId}", f)
}
