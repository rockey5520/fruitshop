...
var _ = Service("user", func() {
        	...
        	Method("add", func() {
                    	Payload(func() {
                                	Field(1, "UserEmailID", String, "UserEmailID")
                                	Field(2, "UserName", String, "User Name")
                                	Required("UserEmailID", "UserName")
                    	})
                    	Result(Empty)
                    	Error("not_found", NotFound, "User not found")
                    	HTTP(func() {
                                	POST("/api/v1/user/{UserEmailID}")
        	...
        	Method("get", func() {
                    	Payload(func() {
                                	Field(1, "UserEmailID", String, "UserEmailID")
                                	Required("UserEmailID")
                    	})
                    	Result(UserManagement)
                    	Error("not_found", NotFound, "User not found")
                    	HTTP(func() {
                                	GET("/api/v1/user/{UserEmailID}")
                                	Response(StatusOK)
                    	})
        	})
...
var UserManagement = ResultType("application/vnd.user", func() {
        	...
        	Attributes(func() {
                    	Attribute("UserEmailID", String, "Email ID is the unique id of the User.", func() {
